...
const patientMainNavigation: NavItem[] = [
  { name: "Overview", href: "/user/overview", icon: LayoutGrid },
  { name: "Appointments", href: "/user/appointments", icon: CalendarDays },
  { name: "Medical Info", href: "/user/medical-info", icon: HeartPulse },
];
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3">
                <Icon
                  className={`h-5 w-5 ${isActive ? "text-brand-red" : "text-gray-500 group-hover:text-brand-red"}`}
                />
                <span>{item.name}</span>
              </div>
              {item.badge ? (
      ...
      <div className="p-4 border-t border-slate-100 space-y-1.5">
        {footerNav.map((item) => {
          ...
          return (
            ...
            >
              <Icon
                className={`h-5 w-5 ${isActive ? "text-brand-red" : "text-gray-400 group-hover:text-brand-red"}`}
              />
              <span>{item.name}</span>
            </Link>
          );
